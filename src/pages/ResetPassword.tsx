...
const ResetPassword: React.FC = () => {
  ...
  useEffect(() => {
    if (!hasToken) setError(t("resetPassword.missingToken"));
  }, [hasToken, t]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token || !newPassword || !confirmPassword) {
      setError(t("resetPassword.fillAll"));
      return;
    }
    ...
    try {
      ...
      setError(e instanceof AdminApiError ? e.message : t("resetPassword.failed"));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center p-8">
        <div className="w-full max-w-sm text-center">
          ...
          <h1 className="mb-2 text-2xl font-bold text-foreground">{t("resetPassword.successTitle")}</h1>
  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-8">
      <div className="w-full max-w-sm">
        ...
        <h1 className="mb-2 text-2xl font-bold text-foreground">{t("resetPassword.setNewTitle")}</h1>
        <p className="mb-6 text-muted-foreground">
          {hasToken ? t("resetPassword.hasTokenHint") : t("resetPassword.noTokenHint")}
        </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex gap-3">
              <Button type="submit" className="flex-1 h-11" disabled={loading}>
                {loading ? t("resetPassword.resetting") : t("resetPassword.resetSubmit")}
              </Button>
