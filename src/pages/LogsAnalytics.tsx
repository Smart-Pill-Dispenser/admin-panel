...
/** Parse "YYYY-MM-DD HH:mm" to Date (date only for range). */
...
const LogsAnalytics: React.FC = () => {
  ...
  const logTypeBg: Record<string, string> = {
    ...
    start: "bg-success/10",
    help: "bg-info/10",
    missed_dose: "bg-warning/10",
    packet_jam: "bg-destructive/10",
    sos: "bg-destructive/10",
    qr_validation: "bg-success/10",
  };
              ...
              {paginated.map((log) => (
                <tr key={log.id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md ${logTypeBg[log.type] ?? "bg-muted"}`}>
