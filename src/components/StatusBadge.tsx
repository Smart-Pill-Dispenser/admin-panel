...
const StatusBadge: React.FC<StatusBadgeProps> = ({ status, className }) => {
  ...
  const cfgClass = statusClass[status] ?? "bg-muted text-muted-foreground";
  const label = t(`status.${status}`, { defaultValue: String(status).replace(/_/g, " ") });
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium", cfgClass, className)}>
